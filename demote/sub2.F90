  ...
  DO JLEV=KTDIA,KLEV
    DO JLON=KIDIA,KFDIA
      IF (YDPHY%LNEIGE) THEN
        ZZPN(JLON) = PLSM(JLON)*PNEIJ(JLON) + (1.0_JPRB - PLSM(JLON))*MAX(0.0_JPRB, SIGN(1.0_JPRB, YDPHY1%TMERGL - PTS(JLON)))
      ELSE
        ZZPN(JLON) = 0.0_JPRB
      END IF
    END DO
    ...
    DO JLON=KIDIA,KFDIA
      ...
      ZZDLS(JLON) = PCHROV(JLON)*(YDCST%RCPD + ZCPVMD*PQS(JLON) + ZCPVMD*PDQSTS(JLON)*PTS(JLON))
      
      ! Derivee du flux de chaleur latente sur eau
      ZZDLLW(JLON) =  &
      & (1.0_JPRB - ZZPN(JLON))*PCHROV(JLON)*((PQS(JLON) - PQ(JLON, KLEV))*ZCPVMW + FOLH(PTS(JLON), 0.0_JPRB)*PDQSTS(JLON))
      
      ! Derivee du flux de chaleur latente sur glace
      ZZDLLS(JLON) = PCHROV(JLON)*ZZPN(JLON)*((PQS(JLON) - PQ(JLON, KLEV))*ZCPVMS + FOLH(PTS(JLON), 1.0_JPRB)*PDQSTS(JLON))
      
      ! Derivee du flux de rayonnement thermique
      ZZDRCN(JLON) = PEMIS(JLON)*4._JPRB*YDCST%RSIGMA*PTS(JLON)*PTS(JLON)*PTS(JLON)
      
      ! Derivee du flux non solaire total
      PDERNSHF(JLON) = -(ZZDLS(JLON) + ZZDLLW(JLON) + ZZDLLS(JLON) + ZZDRCN(JLON))
